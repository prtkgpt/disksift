...
export default function MarketingPage() {
  ...
  return <main>
    <nav className="nav shell"><a className="brand" href="#top"><span className="brand-mark"><SparkIcon /></span>DiskSift</a><div className="nav-links"><a href="#features">Features</a><a href="#how">How it works</a><a href="#pricing">Pricing</a><Link href="/build-in-public">Build in public</Link><Link href="/blog">Blog</Link></div><a className="nav-cta" href="/api/download?edition=free&source=nav">Download for Mac <span>↓</span></a></nav>

    <section className="hero shell" id="top"><div className="eyebrow"><span /> Native macOS app · 100% local</div><h1>Make space for<br/><em>what matters.</em></h1><p className="hero-copy">A calm, private storage analyzer for your Mac. Scan folders locally, find your largest files, and reclaim space safely.</p><div className="hero-actions"><a className="primary" href="/api/download?edition=free&source=hero">Download DiskSift Free <span>↓</span></a><a className="text-link" href="#how">See how it works <span>↓</span></a></div><div className="notarized-badge"><span className="notarized-icon">✓</span><span><strong>Notarized by Apple</strong><small>Developer ID verified · Safe to open with Gatekeeper</small></span></div>
    ...
    <section className="features-section shell" id="features"><div className="features-heading"><div><div className="section-kicker">FEATURES</div><h2>See it. Understand it.<br/>Clear it safely.</h2></div><div><p>DiskSift focuses on the decisions that free meaningful space without turning cleanup into guesswork.</p><a href="#pricing">Compare Free and Pro →</a></div></div><div className="feature-showcase">
      {[
        ["01","Fast local scans","Scan Downloads, your Home folder, or another folder without uploading file names, paths, or contents.","FREE"],
        ["02","Safe to Delete","See conservative, explained recommendations without treating personal documents or media as disposable.","FREE"],
        ["03","Browse and sort","Open every storage category and sort files by size, type, name, or date before acting.","FREE"],
        ["04","Trash-first cleanup","Move reviewed files to macOS Trash instead of deleting them permanently, wherever permissions allow.","FREE"],
        ["05","Exact duplicates","Analyze same-size candidates with SHA-256 content checks instead of relying only on names.","PRO"],
        ["06","Actionable developer cleanup","Measure complete DerivedData, npm, Gradle, Homebrew, and node_modules folders; move supported folders to Trash or ask Xcode to remove unavailable simulators.","PRO"]
      ].map(feature=><article key={feature[0]}><div><span>{feature[0]}</span><b className={feature[3]==="PRO"?"pro-tag":""}>{feature[3]}</b></div><h3>{feature[1]}</h3><p>{feature[2]}</p></article>)}
    </div><div className="features-action"><span>Ready to inspect your Mac?</span><a className="primary" href="/api/download?edition=free&source=features">Download DiskSift Free ↓</a></div></section>
    ...
    <section className="pricing shell" id="pricing"><div className="section-kicker">SIMPLE PRICING</div><h2>Start free. Own Pro forever.</h2><p>The essentials stay free. Upgrade once when you want DiskSift to do the repetitive work for you.</p><div className="one-time-pill">ONE PAYMENT · NO SUBSCRIPTION</div><div className="price-grid">
      <article className="price-card"><div className="plan-head"><span>FREE</span><h3>$0</h3><p>Forever. No account needed.</p></div><a href="/api/download?edition=free&source=pricing" className="plan-button secondary">Download DiskSift Free</a><div className="feature-title">Everything you need to get started</div>{["Full-drive storage overview","Clickable storage categories","Sort by size, type, name, or date","Safe to Delete recommendations","Individual Trash-first cleanup","One Mac"].map(x=><div className="feature" key={x}>✓ <span>{x}</span></div>)}<div className="feature muted">— <span>Batch cleanup plan</span></div><div className="feature muted">— <span>Exact duplicate analysis</span></div></article>
      <article className="price-card pro"><div className="popular">LAUNCH PRICE</div><div className="plan-head"><span>DISKSIFT PRO · LIFETIME</span><h3>$12.99<small> one time</small></h3><p>Pay once. Activate Pro on up to three personal Macs.</p></div><a className="plan-button" href="/buy">Buy DiskSift Pro</a><div className="feature-title">Everything in Free, plus</div>{["Select All Safe and batch cleanup","Warnings for selections not marked safe","Exact duplicates with keeper guidance","Actionable developer cleanup","Three personal Macs","Updates for this major version"].map(x=><div className="feature" key={x}>✦ <span>{x}</span></div>)}<small className="trial-note">15-day money-back guarantee · No subscription</small></article>
    ...
    <section className="faq shell" id="faq"><div><div className="section-kicker">GOOD TO KNOW</div><h2>Questions,<br/>answered.</h2></div><div>{[["Is DiskSift notarized by Apple?","Yes. The public DiskSift DMG is signed with our Apple Developer ID, submitted to Apple’s notary service, and distributed with Apple’s notarization ticket stapled to it."],["Which download works with my Mac?","The single DiskSift download is a universal app containing native Apple silicon and Intel versions."],["Is my scan data uploaded?","No. Scanning, file categorization, duplicate hashing, and cleanup decisions happen locally on your Mac. We do not receive your filenames, paths, contents, or scan results."],["Why might DiskSift request Full Disk Access?","macOS protects certain folders. Full Disk Access is needed only if you choose to analyze protected locations across the full Mac; folder scans can work with the folders you select."],["Why does duplicate analysis take longer?","The initial scan reads fast file metadata. Exact duplicate analysis is optional because it must read candidate files and compare SHA-256 fingerprints byte-for-byte."],["What can I do with DiskSift Free?","Free includes progressive local scans, storage overview, Quick Wins, search, large-file and old-file discovery, plus individual Trash-first actions."],["What does DiskSift Pro add?","Pro adds a cleanup plan, multi-select batch cleanup, exact duplicates with keeper guidance, supported developer-folder cleanup, and activation on up to three personal Macs."],["Will DiskSift delete anything automatically?","No. Cleanup requires your confirmation. Normal file and folder cleanup uses macOS Trash. The separate Xcode unavailable-simulator action is clearly marked because Apple’s tool removes those devices directly."],["Is Pro a subscription?","No. DiskSift Pro is available at a one-time $12.99 launch price. It includes the current Pro features and updates for the purchased major version."],["Can I get a refund?","Yes. Eligible Pro purchases have a 15-day money-back guarantee. See the Refund Policy for details."]].map((f,i)=><div className="faq-item" key={f[0]}><button onClick={()=>setOpenFaq(openFaq===i?-1:i)}><span>{f[0]}</span><b>{openFaq===i?"−":"+"}</b></button>{openFaq===i&&<p>{f[1]}</p>}</div>)}</div></section>
    <footer><div className="shell footer-inner"><div><a className="brand" href="#top"><span className="brand-mark"><SparkIcon /></span>DiskSift</a><p>A clearer way to make space<br/>on your Mac.</p></div><div className="footer-links"><a href="#features">Features</a><a href="#how">How it works</a><a href="#pricing">Pricing</a><Link href="/blog">Mac storage guides</Link><a href="#faq">FAQ</a><Link href="/privacy">Privacy Policy</Link><Link href="/terms">Terms of Service</Link><Link href="/refund-policy">Refund Policy</Link></div><div><small>TRUSTED DOWNLOAD</small><p>✓ Apple notarized<br/>◆ Apple silicon &amp; Intel</p></div></div><div className="shell copyright">© 2026 DiskSift. Built with care for your digital space.<a href="#top">Back to top ↑</a></div></footer>
  </main>;
}
