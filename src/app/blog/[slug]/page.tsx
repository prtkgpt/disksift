...
export default async function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  return <main className="article-page" id="top"><nav className="blog-nav shell"><a className="brand" href="/">DiskSift</a><a href="/blog">← All guides</a></nav><article><header><span>{post.category}</span><h1>{post.title}</h1><p>{post.excerpt}</p><div>By <a href="/authors/prateek-g">{post.authorName}</a> · <time>{post.publishedAt.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</time></div></header><MarkdownArticle source={post.body} /></article><div className="shell"><BlogCta /></div><SiteFooter /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} /></main>;
}
